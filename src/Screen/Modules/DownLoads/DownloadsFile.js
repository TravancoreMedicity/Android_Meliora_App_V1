...
const DownloadsFile = () => {
  ...
  useEffect(() => {
    const getDownloadApi = async () => {
      const getApiInform = await axiosApi.get(
        `/mobileapp/apkDownloadDetails/${1}`
      );
  ...
  return (
    <View>
      ...
      <View
        ...
        style={{
          borderColor: colorTheme.fontColorLightGrey,
          minHeight: 50,
          borderBottomWidth: 1,
        }}
      >
        <View className="flex flex-row items-center grow pl-5 justify-center">
          <CloudArrowDownIcon
            height={35}
            width={35}
            color={colorTheme.greenVarient}
          />
          <Text
            className="pl-2"
            style={{ fontSize: 18, fontFamily: "Roboto_700Bold" }}
          >
            Downloads
          </Text>
        </View>
        <View className="flex w-1/6">
          <TouchableOpacity onPress={navigateToHome} className="flex ">
            <XCircleIcon
              height={30}
              width={30}
              color={colorTheme.fontColorLightGrey}
            />
          </TouchableOpacity>
        </View>
      </View>
